...
const visited = Array(cellsVertical)
    .fill(null)
...
const bfs = (row, column) =>{
    ...
    if(row - 1 >= 0 && horizontals[row-1][column] == true && visited[row-1][column] == false){//Up
        visited[row-1][column] = true;
        console.log("Up " + (row-1) + column);
        queue.push([row-1,column]);
    }
    if(column - 1 >= 0 && verticals[row][column-1] == true && visited[row][column-1] == false){//Left
        visited[row][column-1] = true;
        console.log("Left " + row + (column - 1));
        queue.push([row,column-1]);
    }
    if(row +1 < cellsVertical  && horizontals[row][column] == true && visited[row+1][column] == false ){//Down
        visited[row+1][column] = true;
        console.log("Down " + (row+1) +" " + column);
        queue.push([row+1,column]);
    }
    if(column  < cellsHorizontal && verticals[row][column] == true && visited[row][column+1] == false){//Right
        visited[row][column+1] = true;
